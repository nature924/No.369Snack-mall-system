<template>
  <div class="my-order">
    ...
    <div class="mall-nav">
      <el-menu :default-active="activeIndex" class="el-menu-demo" :collapse-transition="false" active-text-color="#409EFF" mode="horizontal" @select="handleSelect">
        ...
        <el-menu-item index="2" style="width: 50%">
          <el-input placeholder="请输入内容" v-model="queryProductInfo" @keyup.enter.native="queryProduct(queryProductInfo)" class="input-with-select" style="height: 45px;">
            <el-select v-model="select"  slot="prepend" placeholder="请选择" style="width: 120px;">
              ...
              <el-option label="零食品牌" value="3"></el-option>
            </el-select>
        ...
        <el-submenu index="4" style="width: 10%">
          <template slot="title">零食品牌</template>
            ...
            <el-table-column align="center" label="操作">
              <template slot-scope="scope">
                <el-button
                  ...
                  @click="payment(scope.row.orderNo,scope.row.orderTime,scope.row.productName,scope.row.payPrice,scope.row.productType)">零食付款</el-button>
                <el-button
                  size="mini"
    ...
    <el-dialog title="零食评价" class="review" :visible.sync="reviewFormVisible" top="20vh" width="500px">
      <el-form :model="reviewFrom" :rules="reviewRules" ref="reviewFrom">
        <el-form-item label="零食名称" required  prop="productName">
          <el-input size="small" v-model="reviewFrom.productName" readonly placeholder="零食名称"></el-input>
        </el-form-item>
        <el-form-item v-if="reviewFrom.productSpecs" required label="零食规格"  prop="productSpecs">
          <el-input size="small" v-model="reviewFrom.productSpecs" readonly placeholder="零食规格"></el-input>
        </el-form-item>
    ...
    data() {
      ...
      return{
        ...
        reviewRules:{
          ...
          productReview: [{ required: true, message: '请填写零食评价', trigger: 'blur' },
        ...
        this.$refs[formName].validate((valid) => {
            if (valid) {
              ...
              this.$http.post('/productReview/add',this.$qs.stringify(this.productReview,{skipNulls: true})).then((rep)=>{
                ...
                if(rep.data.code===200){
                  this.reqInfo();
                  this.$notify({title: '成功', message: '零食评价提交成功', type: 'success'});
                  this.reviewFormVisible=false;
                  this.$refs[formName].resetFields();
                }else{
